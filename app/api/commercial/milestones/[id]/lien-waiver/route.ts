...
import { attachMilestoneLienWaiver, removeMilestoneLienWaiver } from "@/lib/commercial/invoices/milestones";
...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (String(form.get("remove") ?? "") === "1") {
    const res = await removeMilestoneLienWaiver(id, user.id);
  ...
  const res = await attachMilestoneLienWaiver({
    milestoneId: id,
