...
import { attachAiaLienWaiver, removeAiaLienWaiver } from "@/lib/commercial/aia/lien-waiver";
...
/** Upload (or remove) the lien waiver for one AIA application.
 *
 * A byte-for-byte sibling of the invoice route on purpose (Stephanie: "just as
...
export async function POST(req: Request, ctx: { params: Promise<{ applicationId: string }> }) {
  ...
  if (String(form.get("remove") ?? "") === "1") {
    const res = await removeAiaLienWaiver(id, user.id);
  ...
  const res = await attachAiaLienWaiver({
    applicationId: id,
