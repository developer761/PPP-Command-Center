...
export default async function MessagingSettings({
  ...
  const { data } = await sb
    ...
    .select("id, name, time_zone, quiet_hours_start, quiet_hours_end, send_on_weekends, after_hours_autoreply, after_hours_message, autosend_enabled, reply_delay_min_seconds, reply_delay_max_seconds, reply_to_email")
    .eq("is_active", true)
    .order("name");
