...
async function addJobAction(formData: FormData) {
  ...
  const opportunity_id =
    String(formData.get("opportunity_id") ?? "").trim() || null;

  /**
   * NO OPPORTUNITY PICKED = A ONE-OFF, and it now gets one made for it.
   *
   * Karan 2026-09-17: "if it's a one-off work order it makes a job/opp
   * automatically with some information and tags it as one-off work order."
   *
   * This form has always allowed a blank deal and its own hint invited it —
   * "leave blank for PPP / prevailing-wage / one-off jobs" — which produced a
   * job that schedules and logs hours perfectly and can NEVER be invoiced,
   * because every money table keys on `opportunity_id` and
   * `commercial_invoices.opportunity_id` is NOT NULL. It also appeared in no
   * cost, margin or AR report. You found out when you went to bill it.
   *
   * So the blank path is routed through `createOneOffWorkOrder`, which creates
   * the opportunity, the project and the dashboard work order alongside the
   * job. Same click, same form — the trap is closed for the people already
   * using it rather than avoided by a second form they would have to know to
   * choose.
   */
  if (!opportunity_id) {
    const { createOneOffWorkOrder } =
      await import("@/lib/commercial/field-ops/one-off");
    const oneOff = await createOneOffWorkOrder({
      ...
      division_tag: String(formData.get("division_tag") ?? "") || null,
      notes: String(formData.get("notes") ?? ""),
      actorUserId: userId,
    });
    if (!oneOff.ok)
      redirect(`${BASE}?error=${encodeURIComponent(oneOff.error)}`);
    revalidatePath(BASE);
    redirect(`${BASE}?ok=oneoff`);
  }

  const account_id = await getOpportunityAccountId(opportunity_id);
  const result = await createJob({
    ...
    target_end: String(formData.get("target_end") ?? ""),
    prevailing_wage: formData.get("prevailing_wage") === "on",
    division_tag: (String(formData.get("division_tag") ?? "") ||
      "commercial") as DivisionTag,
    notes: String(formData.get("notes") ?? ""),
...
async function editJobAction(formData: FormData) {
  ...
  const result = await updateJob(
    id,
    {
      ...
      site_city: String(formData.get("site_city") ?? ""),
      status: String(
        formData.get("status") ?? "ready_to_schedule",
      ) as JobStatus,
      estimated_labor_hours: num(formData.get("estimated_labor_hours")),
      target_start: String(formData.get("target_start") ?? ""),
      target_end: String(formData.get("target_end") ?? ""),
...
async function deleteJobAction(formData: FormData) {
  ...
  if (!res.ok)
    redirect(
      `${BASE}?error=${encodeURIComponent(res.error ?? "Could not delete that job.")}`,
    );
  redirect(BASE);
}
...
export default async function FieldOpsJobsPage({
  ...
  await Promise.all([
    ensureJobsForSentWorkOrders(userId),
  ...
  const { getCrewScopeForJob } =
    await import("@/lib/commercial/work-orders/db");
  const scopeByJob = new Map(
    await Promise.all(
      jobs.map(
        async (j) =>
          [j.id, await getCrewScopeForJob(j.id).catch(() => null)] as const,
      ),
    ),
  );
  ...
  return (
    <div className="pb-8 max-w-4xl">
      ...
      {sp.error && (
        <div className="mb-4 rounded-lg bg-rose-50 border border-rose-200 px-3 py-2 text-[12.5px] text-rose-700">
          {sp.error}
        </div>
      )}
      {sp.ok && (
        <div className="mb-4 rounded-lg bg-ppp-green-50 border border-ppp-green-100 px-3 py-2 text-[12.5px] text-ppp-green-700">
          {sp.ok === "added"
            ? "Work order added."
            : sp.ok === "oneoff"
              ? "One-off work order added \u2014 its job was created under Direct / One-off, so it can be costed and invoiced."
              : "Saved."}
        </div>
      )}
      ...
      <form
        action={addJobAction}
        className="bg-surface border border-ppp-charcoal-100 rounded-xl p-4 mb-5 space-y-3"
      >
        ...
        {dealOptions.length > 0 && (
          <div>
            <span className={LABEL_CLS}>
              Connect to an opportunity (optional)
            </span>
            <SearchableSelect
              name="opportunity_id"
              ariaLabel="Connect to an opportunity"
              options={dealOptions}
              placeholder="Search an opportunity / GC to link it"
            />
            <p className="text-[11px] text-ppp-charcoal-400 mt-1">
              Links this work order to that account and opportunity, like a real
              WO.{" "}
              <span className="text-ppp-charcoal-600">
                Leave blank for a one-off — we&rsquo;ll create the job for you
                under <span className="font-semibold">Direct / One-off</span>,
                so it can be costed and invoiced like any other.
              </span>
            </p>
          </div>
        )}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <label className="block sm:col-span-2">
            ...
            <input
              name="name"
              required
              placeholder="Stark Enterprises — lobby & halls"
              className={INPUT_CLS}
            />
          </label>
          <label className="block">
            <span className={LABEL_CLS}>Customer</span>
            <input
              name="customer_name"
          ...
          <label className="block">
            <span className={LABEL_CLS}>Division</span>
            <select
              name="division_tag"
          ...
          <label className="block">
            ...
            <select
              name="status"
              defaultValue="ready_to_schedule"
              className={SELECT_CLS}
              style={SELECT_BG_STYLE}
            >
              {JOB_STATUSES.map((s) => (
          ...
          <label className="block">
            <span className={LABEL_CLS}>Est. labor hours</span>
            <input
          ...
          <div>
            <span className={LABEL_CLS}>Target start</span>
            <DateField
          ...
          <div>
            <span className={LABEL_CLS}>Target end</span>
            <DateField
        ...
        <div className="flex items-center gap-1.5">
          <label className="flex items-center gap-2 text-base sm:text-[13px] text-ppp-charcoal-700 min-h-[44px] sm:min-h-0">
            <input type="checkbox" name="prevailing_wage" className="h-4 w-4" />{" "}
            Prevailing wage (PW)
          </label>
          <InfoDot text={PW_HELP} />
        </div>
        <label className="block">
          <span className={LABEL_CLS}>Notes</span>
          <textarea name="notes" rows={2} className={TEXTAREA_CLS} />
        </label>
        <SubmitButton className="inline-flex items-center px-4 py-2 rounded-lg bg-cc-brand-600 text-white text-[13px] font-semibold hover:bg-cc-brand-700 min-h-[44px]">
          Add work order
        </SubmitButton>
      </form>
      ...
      <div className="flex items-center justify-between mb-2">
        ...
        <Link
          ...
          className="text-[12px] font-semibold text-cc-brand-700 hover:underline"
        >
          {sp.closed === "1" ? "Hide closed" : "Show closed"}
        </Link>
      </div>
      ...
      {jobs.length === 0 ? (
        <div className="text-center py-10 bg-surface border border-ppp-charcoal-100 rounded-xl">
          ...
          <p className="text-[12.5px] text-ppp-charcoal-500 mt-1">
            Send an opportunity&rsquo;s work order to Field Ops, or add a
            one-off above — then schedule the crew onto it.
          </p>
        </div>
      ) : (
        <ul className="space-y-2">
          {jobs.map((j) => {
            ...
            return (
              <li
                key={j.id}
                className="bg-surface border border-ppp-charcoal-100 rounded-xl"
              >
                <details>
                  <summary className="flex items-center gap-3 px-4 py-3 cursor-pointer list-none min-h-[52px]">
                    <div className="min-w-0 flex-1">
                      <div className="text-[13.5px] font-semibold text-ppp-charcoal truncate">
                        {j.name}{" "}
                        {j.prevailing_wage && (
                          <span className="ml-1 text-[10px] font-bold bg-ppp-charcoal-100 text-ppp-navy rounded px-1">
                            PW
                          </span>
                        )}
                      </div>
                      <div className="text-[11.5px] text-ppp-charcoal-500 truncate font-mono">
                        {j.job_code} · {jobStatusLabel(j.status)}
                        {j.division_tag
                          ? ` · ${divisionLabel(j.division_tag)}`
                          : ""}
                      ...
                      {scope && scope.lines.length > 0 && (
                        <div className="mt-1.5 text-[11.5px] text-ppp-charcoal-600">
                          <span className="font-semibold">
                            ...
                            Scope
                            {scope.isPartial
                              ? ` (${scope.lines.length} of ${scope.totalLines})`
                              : ""}
                            :
                          </span>
                          <ul className="list-disc pl-4 mt-0.5 space-y-0.5">
                            ...
                            {scope.lines.length > 6 && (
                              <li>…and {scope.lines.length - 6} more</li>
                            )}
                          </ul>
                          {scope.isPartial && (
                            <div className="font-semibold text-amber-800 mt-0.5">
                              These items only — the rest is on another work
                              order.
                            </div>
                          )}
                        </div>
                  ...
                  <form
                    ...
                    className="px-4 pb-4 pt-1 space-y-3 border-t border-ppp-charcoal-50"
                  >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      ...
                      <label className="block">
                        ...
                        <input
                          ...
                          min="0"
                          step="1"
                          defaultValue={j.estimated_labor_hours ?? ""}
                          className={INPUT_CLS}
                        />
                      </label>
                      <label className="block sm:col-span-2">
                    ...
                    <div className="flex items-center gap-1.5">
                      <label className="flex items-center gap-2 text-base sm:text-[13px] text-ppp-charcoal-700 min-h-[44px] sm:min-h-0">
                        <input
                          ...
                          defaultChecked={j.prevailing_wage}
                          className="h-4 w-4"
                        />{" "}
                        Prevailing wage (PW)
                      </label>
                    ...
                    <SubmitButton className="inline-flex items-center px-4 py-2 rounded-lg bg-cc-brand-600 text-white text-[12.5px] font-semibold hover:bg-cc-brand-700 min-h-[44px]">
                      Save
                    </SubmitButton>
                  </form>
