...
import {
  normalizeToolOrigin,
  toolOriginQs,
} from "@/lib/commercial/tool-origin";
import {
  DonutChart,
  GaugeRing,
  type ChartTone,
...
function costsRedirect(
  ...
  if (
    back &&
    (back.startsWith("/commercial/post-job/") ||
      back.startsWith("/commercial/invoices/new?opp="))
  )
    p.back = back;
...
const CATEGORY_OPTIONS: [string, string][] = OFFERED_PURCHASE_CATEGORIES.map(
  (c) => [c, PURCHASE_CATEGORY_META[c].label],
);
...
async function readReceiptFile(
  formData: FormData,
): Promise<{ file_name: string; mime_type: string; data: Uint8Array } | null> {
...
const RECEIPT_FAILED_NOTE =
  "Purchase saved, but the receipt didn't upload — add it from Edit.";
...
async function addPurchaseAction(formData: FormData) {
  ...
  if (!UUID_RE.test(opp_id) || !UUID_RE.test(account_id))
  ...
  const preserve = {
    pu_cat: category,
    pu_vendor: vendor.slice(0, 200),
    pu_amt: rawAmount.slice(0, 40),
    pu_hours: rawHours.slice(0, 20),
    pu_date: rawDate.slice(0, 10),
    pu_desc: description.slice(0, 1000),
    pu_reimb: reimburseTo.slice(0, 120),
    ...vendorPick.preserve,
  };
  ...
  if (cents === null || cents <= 0) {
    costsRedirect(
      account_id,
      opp_id,
      { error: "Enter a transaction amount greater than $0.", ...preserve },
      back,
      origin,
      from,
    );
  ...
  const v = await resolvePurchaseVendor({
    vendorId: vendorPick.id,
    vendorText: vendor,
    category,
    createIfNew: vendorPick.createNew,
    userId,
  });
  ...
  costsRedirect(
    ...
    {
      cost_ok: "added",
      ...(receiptFailed ? { heads_up: RECEIPT_FAILED_NOTE } : {}),
    },
    back,
    origin,
    from,
...
async function updatePurchaseAction(formData: FormData) {
  ...
  if (
    !UUID_RE.test(opp_id) ||
    !UUID_RE.test(account_id) ||
    !UUID_RE.test(purchase_id)
  )
    redirect("/commercial/accounts");
  await assertDealOwned(opp_id, account_id);
  const category = String(formData.get("category") ?? "materials");
  ...
  if (cents === null || cents <= 0) {
    costsRedirect(
      ...
      opp_id,
      {
        error: "Enter a transaction amount greater than $0.",
        edit_purchase: purchase_id,
        ...preserve,
      },
      back,
      origin,
  ...
  if (!res.ok)
    costsRedirect(
      account_id,
      opp_id,
      { error: res.error, edit_purchase: purchase_id, ...preserve },
      back,
      origin,
      from,
    );
...
async function deletePurchaseAction(formData: FormData) {
  ...
  if (!res.ok)
    costsRedirect(account_id, opp_id, { error: res.error }, back, origin, from);
  revalidateCostSurfaces(account_id, opp_id);
...
function marginTone(pct: number | null): {
  ...
  if (pct == null)
    return {
      text: "text-ppp-charcoal-400",
      bar: "bg-ppp-charcoal-200",
      label: "—",
    };
  if (pct < 0)
    return { text: "text-rose-700", bar: "bg-rose-500", label: "over budget" };
  if (pct < 15)
    return {
      text: "text-amber-700",
      bar: "bg-amber-500",
...
export async function ProjectCostsTool({
  ...
  const truePctOfContract = fin.hasContract
    ? Math.round((totalCostCents / fin.contractCents) * 100)
    : 0;
  ...
  /**
   * Spend per supplier on this job — Stephanie's waiver question.
   *
   * Built from `purchases`, already loaded above, rather than a new query.
   *
   * `labor` is EXCLUDED: a GC waiver covers material suppliers, and listing
   * "Tomco Labor - Greg" beside Sherwin Williams would present a crew payout as
   * a supplier invoice. Crew money keeps its own section.
   *
   * Grouped case-insensitively on a trimmed name, because the vendor field is
   * free text as well as a directory pick — "Aboffs" and "aboffs " are one
   * supplier on a waiver and would otherwise be two lines that each look wrong.
   */
  const vendorSpend = (() => {
    const by = new Map<
      string,
    ...
    for (const p of purchases) {
      ...
      const cur = by.get(key) ?? { vendor: name, cents: 0, count: 0 };
  ...
  const vendorSpendTotal = vendorSpend.reduce((n, v) => n + v.cents, 0);
  ...
  const costSegments: DonutSegment[] = [
    ...
    ...(crewLaborTotalCents > 0
      ? [
          {
            label: "Crew labor",
            value: crewLaborTotalCents,
            tone: CREW_LABOR_TONE,
            valueLabel: formatCentsCompact(crewLaborTotalCents),
          },
        ]
      : []),
  ];
    ...
    <div className="space-y-3">
      {sp.cost_ok && COST_OK_MESSAGES[sp.cost_ok] ? (
        <div className="rounded-lg px-4 py-3 text-sm flex items-start justify-between gap-3 bg-emerald-50 border border-emerald-200 text-emerald-800">
          ...
          <Link
            href={`${costsBase(id, dealId, variant)}${toolOriginQs(sp.from)}`}
      ...
      <section className="bg-gradient-to-br from-cc-brand-50/60 to-surface border border-cc-brand-100 rounded-xl p-4 sm:p-5">
        <div className="flex items-center gap-2 mb-3">
          <span
            aria-hidden
            className="inline-flex items-center justify-center h-8 w-8 rounded-lg bg-cc-brand-600 text-white"
          >
            <svg
              width="16"
              height="16"
          ...
          <div>
            ...
            <p className="text-[11px] text-ppp-charcoal-500 leading-snug">
              Contract minus job costs. What we bill the GC never changes with
              cost.
            </p>
          </div>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
          <PLTile
            label="Contract"
            value={fin.hasContract ? formatCentsFull(fin.contractCents) : "—"}
            hint={fin.hasContract ? undefined : "Set a proposal/bid"}
          />
          <PLTile label="Invoiced" value={formatCentsFull(fin.invoicedCents)} />
          <PLTile
            label="Collected"
            value={formatCentsFull(fin.collectedCents)}
            tone="emerald"
          />
          <PLTile
            label="Costs"
            value={formatCentsFull(totalCostCents)}
            tone={totalCostCents > 0 ? "rose" : "neutral"}
          />
          <PLTile
        ...
        {fin.hasContract && (
          <div className="mt-3">
            <div className="flex items-center justify-between text-[10.5px] text-ppp-charcoal-500 mb-1">
              ...
              <span className={`font-semibold ${mt.text}`}>
                {truePctOfContract}% spent
              </span>
            </div>
            <div className="h-2.5 rounded-full bg-ppp-charcoal-100 overflow-hidden">
              <div
                className={`h-full rounded-full ${mt.bar}`}
                style={{ width: `${barPctOfContract}%` }}
              />
            </div>
          </div>
        )}
        ...
        {totalCostCents > 0 && (
          <div className="mt-3 flex flex-wrap gap-1.5">
            {PURCHASE_CATEGORIES.filter((c) => fin.costs[c] > 0).map((c) => (
              <span
                key={c}
                className="inline-flex items-center gap-1.5 rounded-lg border border-ppp-charcoal-100 bg-surface px-2.5 py-1 text-[11px]"
              >
                <span className="font-semibold text-ppp-charcoal-600">
                  {PURCHASE_CATEGORY_META[c].label}
                </span>
                <span className="tabular-nums font-bold text-ppp-charcoal">
                  {formatCentsFull(fin.costs[c])}
                </span>
                <span className="text-ppp-charcoal-400 tabular-nums">
                  {Math.round((fin.costs[c] / totalCostCents) * 100)}%
                </span>
              </span>
            ))}
            {crewLaborTotalCents > 0 && (
              <span className="inline-flex items-center gap-1.5 rounded-lg border border-emerald-200 bg-emerald-50 px-2.5 py-1 text-[11px]">
                ...
                <span className="tabular-nums font-bold text-ppp-charcoal">
                  {formatCentsFull(crewLaborTotalCents)}
                </span>
                <span className="text-ppp-charcoal-400 tabular-nums">
                  {Math.round((crewLaborTotalCents / totalCostCents) * 100)}%
                </span>
              </span>
            )}
          </div>
      ...
      {(totalCostCents > 0 || fin.billedPreTaxCents > 0) && (
        <section className="bg-surface border border-ppp-charcoal-100 rounded-xl p-4 sm:p-5">
          <h3 className="text-[13px] font-bold text-ppp-charcoal mb-3 flex items-center gap-2">
            <span
              aria-hidden
              className="inline-block h-[3px] w-6 rounded-full bg-cc-brand-600"
            />
            Revenue &amp; margin
          </h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 items-center">
            <div className="flex items-center justify-center">
              {costSegments.length > 0 ? (
                <DonutChart
                  ...
                  centerValue={formatCentsCompact(totalCostCents)}
                  centerLabel="job costs"
                />
              ) : (
                <p className="text-[12px] text-ppp-charcoal-400 text-center">
                  No costs logged yet — add one below.
                </p>
              )}
            </div>
            <div className="flex items-center gap-4">
              <GaugeRing
                ...
                tone={
                  dm.provisional || billedMarginPct === null
                    ? "neutral"
                    : billedMarginPct < 0
                      ? "rose"
                      : billedMarginPct < 15
              ...
              <div className="min-w-0 text-[12px] space-y-1">
                <div>
                  ...
                  <strong className="tabular-nums text-ppp-charcoal">
                    {formatCentsCompact(fin.billedPreTaxCents)}
                  </strong>
                </div>
                <div>
                  <span className="text-ppp-charcoal-500">Costs: </span>
                  <strong className="tabular-nums text-ppp-charcoal">
                    {formatCentsCompact(totalCostCents)}
                  </strong>
                </div>
                <div className="pt-1 border-t border-ppp-charcoal-100">
                  <span className="text-ppp-charcoal-500">Net profit: </span>
                  <strong
                    className={`tabular-nums ${netProfitCents < 0 ? "text-rose-700" : "text-emerald-700"}`}
                  >
                    {netProfitCents < 0 ? "−" : ""}
                    {formatCentsCompact(Math.abs(netProfitCents))}
                  </strong>
                </div>
              </div>
            </div>
              ...
              >
                <div className="min-w-0">
                  ...
                  <div className="text-[11px] text-ppp-charcoal-400 tabular-nums">
                    {w.hours.toLocaleString("en-US", {
                      maximumFractionDigits: 2,
                    })}{" "}
                    hrs
                    {w.currentRateCents != null
                      ? ` · ${formatCentsFull(w.currentRateCents)}/hr`
                      : ""}
                    {w.unratedHours > 0 && (
                      <span className="text-amber-700">
                        {" "}
                        ·{" "}
                        {w.unratedHours.toLocaleString("en-US", {
                          maximumFractionDigits: 2,
                        })}{" "}
                        hrs unrated
                      </span>
                ...
                <div className="text-[13px] font-bold tabular-nums text-ppp-charcoal shrink-0">
                  {w.isSub ? (
                    <span className="font-semibold text-[11px] text-ppp-charcoal-400 italic">
                      via crew payout
                    </span>
                  ) : (
      ...
      {/* ── SPEND BY VENDOR ──
          Stephanie via Katie, 2026-09-18: "Can we add a job cost by vendor
          report? Where we can see how much total we spent at each vendor for a
          specific job… There are times when we have to complete reporting and
          waivers for GC showing total material costs by vendor."

          She asked for it on the job AND in Accounting; this is the job half.
          Computed from `purchases`, which this page already loads — the deal
          page is the slowest surface in the platform and a vendor rollup is not
          worth another round trip.

          LABOR IS EXCLUDED on purpose. A GC waiver is about MATERIAL suppliers,
          and folding crew payouts into a vendor list would put "Tomco Labor -
          Greg" beside Sherwin Williams as though the two were the same kind of
          thing. Crew payouts have their own section below. */}
        ...
        <section className="bg-surface border border-ppp-charcoal-100 rounded-xl p-4 sm:p-5">
          <div className="flex items-center justify-between gap-2 mb-2.5">
            ...
            <span className="text-[11px] text-ppp-charcoal-500 tabular-nums">
              {formatCentsFull(vendorSpendTotal)} across {vendorSpend.length}{" "}
              {vendorSpend.length === 1 ? "vendor" : "vendors"}
            </span>
          </div>
          <ul className="divide-y divide-ppp-charcoal-100">
            {vendorSpend.map((v) => (
              ...
              >
                <div className="min-w-0">
                  ...
                  <div className="text-[11px] text-ppp-charcoal-400 tabular-nums">
                    {v.count} {v.count === 1 ? "purchase" : "purchases"}
                  </div>
                </div>
                <div className="text-[13px] font-bold tabular-nums text-ppp-charcoal shrink-0">
                  {formatCentsFull(v.cents)}
                </div>
              </li>
            ))}
          </ul>
          <p className="mt-2.5 text-[11px] text-ppp-charcoal-400 leading-snug">
            Materials and other supplier spend on this job. Crew payouts are
            below, and are not supplier costs.
          </p>
        </section>
      )}
                ...
                <div className="min-w-0">
                  ...
                  <div className="text-[11px] text-ppp-charcoal-400 tabular-nums">
                    {w.hours > 0
                      ...
                      : "hours not logged"}
                    {w.rate_cents_per_hour != null
                      ? ` · ${formatCentsFull(w.rate_cents_per_hour)}/hr`
                      : ""}
      ...
      <section className="bg-surface border border-ppp-charcoal-100 rounded-xl p-4 sm:p-5">
        {purchases.length === 0 && (
          <p className="text-[12px] text-ppp-charcoal-500 mb-3">
            No costs logged yet. Add materials, labor, subs, equipment or
            permits below to see this job&rsquo;s margin.
          </p>
        )}
        <details
          className="group mb-3 rounded-lg"
          open={(!!sp.error && !editId) || purchases.length === 0}
        >
          ...
          <PurchaseForm
            ...
            preserve={{
              ...
              vendor: sp.pu_vendor,
              vendorId: sp.pu_vid,
              vendorNew: sp.pu_vnew === "1",
              amt: sp.pu_amt,
              hours: sp.pu_hours,
              date: sp.pu_date,
              desc: sp.pu_desc,
              reimburseTo: sp.pu_reimb,
            }}
          />
        </details>
        ...
        {purchases.length > 0 && (
          <ul className="space-y-2.5">
            {purchases.map((pu, puIdx) => {
              ...
              const receipt = pu.receipt_document_id
                ? (receiptDocs.get(pu.receipt_document_id) ?? null)
                : null;
              const meta =
                PURCHASE_CATEGORY_META[pu.category] ??
                PURCHASE_CATEGORY_META.other;
              return (
                <li
                  key={pu.id}
                  className="border border-ppp-charcoal-100 rounded-lg p-3 sm:p-3.5"
                >
                  {isEditing ? (
                    <PurchaseForm
                    ...
                    <div className="flex items-start justify-between gap-3 flex-wrap">
                      <div className="min-w-0">
                        <div className="flex items-center gap-2 flex-wrap">
                          {transId && (
                            <span
                              className="text-[9.5px] font-mono text-ppp-navy-600"
                              title="Transaction ID — shares this project's number"
                            >
                              {transId}
                            </span>
                          )}
                          <span className="inline-flex items-center px-2 py-0.5 rounded-full border border-ppp-charcoal-200 bg-ppp-charcoal-50 text-[11px] font-semibold text-ppp-charcoal-700">
                            {purchaseCategoryLabel(pu.category)}
                          </span>
                          {pu.vendor && (
                        ...
                        {pu.description && (
                          <div className="text-[12px] text-ppp-charcoal-500 mt-0.5 break-words whitespace-pre-wrap">
                            {pu.description}
                          </div>
                        )}
                        <div className="text-[11px] text-ppp-charcoal-400 mt-1 flex items-center gap-2 flex-wrap">
                          ...
                          {receipt && (
                            <a
                              href={`/api/commercial/documents/${receipt.id}/download`}
                              target="_blank"
                              rel="noopener noreferrer"
                              className="inline-flex items-center gap-1 text-cc-brand-700 hover:text-cc-brand-800 min-h-[44px]"
                            >
                              <svg
                                width="11"
                                height="11"
                      ...
                      <div className="flex flex-col items-end gap-1 shrink-0">
                        <div
                          className={`text-base font-bold tabular-nums text-ppp-charcoal`}
                        >
                          −{formatCentsFull(pu.amount_cents)}
                        </div>
                        <div className="flex items-center gap-1">
                          <Link
                            href={`${`${costsBase(id, dealId, variant)}${toolOriginQs(sp.from)}`}&edit_purchase=${pu.id}`}
                            className="inline-flex items-center px-2.5 py-1.5 rounded-lg border border-ppp-charcoal-200 text-[12px] font-medium text-ppp-charcoal hover:bg-ppp-charcoal-50 min-h-[44px]"
                          >
                            Edit
                          </Link>
                          <form action={deletePurchaseAction}>
                            ...
                            />
                            <ConfirmSubmitButton
                              message={`Delete this ${purchaseCategoryLabel(pu.category).toLowerCase()} transaction? This can't be undone.`}
                              pendingLabel="Deleting…"
                              className="inline-flex items-center px-2.5 py-1.5 rounded-lg text-[12px] font-medium text-ppp-charcoal-400 hover:text-rose-700 hover:bg-rose-50 min-h-[44px]"
                            >
                              Delete
                            </ConfirmSubmitButton>
                          </form>
...
function PLTile({
  ...
  return (
    <div
      className={`rounded-lg border px-2.5 py-2 ${emphasize ? "border-cc-brand-300 bg-surface" : "border-ppp-charcoal-100 bg-surface/70"}`}
    >
      <div className="text-[11px] font-bold uppercase tracking-wide text-ppp-charcoal-500">
        {label}
      </div>
      <div
        className={`font-condensed text-lg sm:text-xl font-black tabular-nums leading-none mt-0.5 ${valueCls}`}
      >
        {value}
      </div>
      {sub && <div className={`text-[11px] mt-0.5 ${valueCls}`}>{sub}</div>}
      {hint && (
        <div className="text-[11px] text-ppp-charcoal-500 mt-0.5">{hint}</div>
      )}
    </div>
  );
}
