...
- ⬜ **03 · Layout** — Rework to the mockup: remove **Reference tab**; move buttons out of Customer/Materials tabs into ONE **Button Bar** (split **Colors** vs **Materials** by bg/heading) → removes those two tabs; rename "Preview Colors" → **"Preview Materials Order"**; "Order materials" → **"Order Materials"**; add **Follow-up Date** mirroring the WO's SF Follow-up Date (editable + writeback); keep "Rooms and Colors" preview ("colors not entered yet" when empty).
