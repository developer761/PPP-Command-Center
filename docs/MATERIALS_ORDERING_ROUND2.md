...
- 🔨 **03 · Layout** — ✅ Removed Reference tab; ✅ tabs → single **Button Bar** with **Colors** (blue) + **Materials** (green) sections; ✅ Internal Entry moved into the Colors bar; ✅ "Preview colors" → **"Preview Materials Order"**; ✅ "Order materials" → **"Order Materials"** (both desktop + mobile); ✅ "Rooms and colors" preview already present. ⬜ **Follow-up Date** (SF `FollowupDate__c` read + editable + writeback) — next.
