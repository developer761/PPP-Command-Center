...
### R36. F8 is a PARTIAL — the won deal's own overview still has no Start-Project action (Katie's one-click/under-the-deal bar)
`98b43be` verified: F7 (drag-to-Proposal now rejects with a loud 409, no write-then-revert), F10 (due countdown uses ET calendar days, bare-date pass-through — matches the badge), F11 (reopen-expired gets its own "Reopened" banner + carries proposalBack). All correct.
F8 half-landed: the **debrief page** Start Project is now un-gated from `isDebriefed` and a won debrief keeps you there (`submitDebriefAction` redirects `?saved=1`) — good. But the **won_not_started overview panel** (`accounts/[id]/page.tsx:1528-1552`) is still **prose-only** ("Move it to Pre-Construction", no button/link) — the commit's own comment says "with no button anywhere" — so from the deal's own overview a user still can't one-click Start Project; they must find the debrief pill or the status dropdown. Also still hardcodes **"Nothing is billed yet"** even when a deposit invoice exists (R10, unfixed).
**Fix (Katie's bar — under-the-deal, one-click):** put a real **"Start project → Pre-Construction"** button on the won_not_started panel (wired to the same action the debrief page uses), and gate the "Nothing is billed yet" line on `dealFin.billedPreTaxCents===0` (R10).

### Still OPEN after this batch (not deferred — next up)
...
### R44. R26 is still 90% — the account-LIST bid range wasn't given the fallback (only the scorecard was)
`f8a994f` gave the account **scorecard** (`accounts/[id]/page.tsx`) the proposal-total fallback (verified — `listCurrentProposalTotalByOpp` + `dealValueCents` → correct). But it did **not** touch the accounts-**list** page (`accounts/page.tsx`), which still reads the raw SQL-view columns with no fallback: the book-level "Bid range · across matches" tile (`:273-274` sum → `:406` render) and the per-account `AccountRow` bid range (`:1054-1056` `formatBidCents(total_active_bid_low/high_cents)`). So the accounts list still shows **"Open bids: 3" next to "Bid range: —"** for priced-but-bid-less deals — the exact R26 bug, one level up. My original R26 finding listed both consumers (`accounts/page.tsx:273-274,1056` AND `accounts/[id]/page.tsx:7526-7527`); only the second was done.
**Fix:** batch `listCurrentProposalTotalByOpp` over the displayed accounts' open opps and sum `dealValueCents(o, total)` for the book tile + AccountRow — same as the scorecard fix, one level up. (R28 is fully complete — all three Net-profit cards go neutral-when-provisional.)
