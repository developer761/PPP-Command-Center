...
describe("effectiveReportRole — an admin flag beats a stale role string", () => {
  const noFolders = async () => ({ folders: [], items: [], memberships: [] });

  it("is_admin=true with role='rep' still sees every report", async () => {
    // Karan's own commercial login is exactly this. Before the fix he would
    // have opened Reports after the migration and found it empty.
    const role = effectiveReportRole({ role: "rep", isAdminFlag: true });
    expect(role).toBe("admin");
    const res = await computeReportAccess({ userId: "u1", role }, noFolders);
    expect(res.visible.length).toBe(REPORTS.length);
  });

  it("without the flag the role string stands, and folders decide", async () => {
    expect(effectiveReportRole({ role: "rep", isAdminFlag: false })).toBe("rep");
    const res = await computeReportAccess({ userId: "u1", role: "rep" }, noFolders);
