    ...
    <div className={`overflow-x-auto -mx-1 px-1 ${className}`}>
      <div className="flex gap-2 min-w-full">
        {segments.map((s, i) => {
          ...
          const fill = fullyPaid ? "bg-emerald-500" : partial ? "bg-ppp-blue-500" : "bg-ppp-charcoal-200";
          const dueCls = s.overdue ? "text-amber-700 font-semibold" : "text-ppp-charcoal-400";
          return (
            <div key={i} className="flex flex-col min-w-[76px]" style={{ flexGrow: Math.max(1, s.amountCents) }}>
              {/* Mini-header — name + due */}
              <div className="mb-1 min-w-0">
                <div className="text-[10.5px] font-semibold text-ppp-charcoal truncate" title={s.name}>{s.name}</div>
                <div className={`text-[9.5px] ${dueCls} truncate`}>{s.due ? `Due ${s.due}` : "No due date"}</div>
              </div>
              {/* Chunk */}
              <div className="h-2.5 rounded-full bg-ppp-charcoal-100 overflow-hidden relative" title={`${fmt(s.paidCents)} of ${fmt(s.amountCents)}`}>
              ...
              <div className="mt-1 flex items-center justify-between gap-1 text-[9.5px]">
                <span className="tabular-nums font-semibold text-ppp-charcoal-600">{fmt(s.amountCents)}</span>
                {fullyPaid ? (
