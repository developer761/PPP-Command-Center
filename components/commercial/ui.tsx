...
/**
 * The one tool/section header: a tinted strip with a blue icon disc + title +
 * optional subtitle and a right-aligned action slot. The single header shape
 * for the delivery tools (replaces each tool's bespoke header + divergent
 * accents, e.g. the Submittals-blue one).
 */
  ...
  return (
    <div className={`flex items-start justify-between gap-3 ${className}`}>
      <div className="flex items-start gap-2.5 min-w-0">
        {icon && (
          <span aria-hidden className="mt-0.5 inline-flex items-center justify-center h-8 w-8 rounded-lg bg-cc-brand-50 text-cc-brand-700 shrink-0">
            {icon}
          </span>
        )}
        <div className="min-w-0">
          <h2 className="text-[15px] font-bold text-ppp-charcoal leading-tight">{title}</h2>
          {subtitle && <p className="text-[12px] text-ppp-charcoal-500 mt-0.5 leading-snug">{subtitle}</p>}
        </div>
      </div>
