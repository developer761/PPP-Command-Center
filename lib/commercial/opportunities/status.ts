...
export function shouldWarnTransition(
  from: OpportunityStatus,
  to: OpportunityStatus,
): boolean {
...
> {
  ...
  const effectiveSubStatus =
    input.to_sub_status !== undefined && input.to_sub_status !== null
      ? input.to_sub_status
      : ((DEFAULT_SUB_STATUS_BY_STATUS as Record<string, string>)[
          input.to_status
        ] ?? null);
  if (
  ...
  if (targetIsLost) {
    if (input._skipDagCheck) {
      ...
      lossReason = "other";
      lossNote =
        "Auto-set by cascade — complete the debrief form to record the real reason.";
    } else {
      if (
        !input.loss_reason ||
        !OPPORTUNITY_LOSS_REASONS.includes(input.loss_reason)
      ) {
        return {
          ok: false,
          error:
            "Pick a reason for losing (or `no_bid` if we declined to bid).",
        };
      }
  ...
  const fromDefault =
    DEFAULT_PROBABILITY_BY_SUB_STATUS[beforeRow.sub_status ?? ""] ?? null;
  const userOverrode =
    fromDefault !== null && beforeRow.probability_pct !== fromDefault;
  const preserveProbability = PROBABILITY_PRESERVING_SUB_STATUSES.has(
    effectiveSubStatus ?? "",
  );
  const nextProbability =
    userOverrode || preserveProbability
      ? beforeRow.probability_pct
      : (DEFAULT_PROBABILITY_BY_SUB_STATUS[effectiveSubStatus ?? ""] ??
        beforeRow.probability_pct);
  ...
  const toInDelivery =
    IN_DELIVERY_STATUSES.includes(input.to_status) ||
    input.to_status === "post_sale_closed";

  let nextDecidedAt: string | null | undefined = undefined; // undefined = don't touch
  if (
    toPreSaleClosed &&
    (!wasPreSaleClosed || beforeRow.sub_status !== effectiveSubStatus)
  ) {
  ...
  const nextSubStatus =
    input.to_sub_status &&
    isValidSubStatus(input.to_status, input.to_sub_status)
      ? input.to_sub_status
      : ((DEFAULT_SUB_STATUS_BY_STATUS as Record<string, string>)[
          input.to_status
        ] ?? "rfp");
  ...
  if ((input.source ?? "user") === "user")
    patch.status_user_set_at = new Date().toISOString();
  ...
  if (
    updateErr &&
    /status_user_set_at|closed_out_at/i.test(updateErr.message)
  ) {
    console.warn(
    ...
    try {
      const deriveTargetProposalStatus = (): {
        ...
        if (s === "estimating" && sub === "proposal_pending_approval") {
          ...
          return {
            demoteFrom: ["draft", "approved", "sent", "won", "lost"],
            to: "pending_approval",
          };
        }
        if (s === "proposal") {
      ...
      if (target) {
        const { updateProposalStatus, requestProposalApproval } =
        ...
        const proposals =
          (propRows as
            { id: string; status: string; revision_number: number }[] | null) ??
          [];
        for (const p of proposals) {
          ...
          const flip = await updateProposalStatus({
            id: p.id,
            to_status: target.to as Parameters<
  ...
  // couldn't be written. Idempotent, so the next status change re-attempts.
  {
    ...
    if (isProjectBearing || wasProjectBearing) {
      try {
        ...
        if (!res.ok) {
          console.warn(
            "[changeOpportunityStatus] project ensure failed:",
            res.error,
          );
        }
      } catch (err) {
