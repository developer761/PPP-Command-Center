...
-- Principles: IDs are keys (never names); scheduled != actual (separate tables);
-- crews are templates (assignments hold hours, crews do not); job_code mandatory;
-- pay rates isolated in their own gated table.
...
-- Job phases (multiple date ranges per job)
...
-- At most ONE open punch per employee (cannot be clocked into two things at once).
...
-- Internal schedule-email recipients (office: Stephanie/Brendan/etc)
